...
OUTPUT = ROOT / "artifacts" / "财务通用Agent-最终用户手册.docx"
...
def style_document(doc):
    ...
    hr = hp.add_run("财务通用 Agent  |  最终用户手册")
    ...
    fr = fp.add_run("finance-general-agent  |  V1.0  |  2026-06-04")
    set_font(fr, 9, MUTED)
...
def add_cover(doc):
    ...
    r = p.add_run("查询财务数据 · 处理发票与报销 · 生成凭证与报表 · 风险与资金分析")
    set_font(r, 11.5, MUTED)

    table = doc.add_table(rows=2, cols=3)
    set_table_geometry(table, [3000, 3000, 3000], indent_dxa=180)
    labels = [("76", "聚合工具"), ("45", "财务工具"), ("4", "业务页面"), ("22", "浏览器工具"), ("8", "多模态工具"), ("1", "统一入口")]
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p.add_run("适用于财务人员、业务审批人员、管理者和系统管理员")
...
def add_quick_navigation(doc):
    ...
    add_inline(intro, "首次使用建议依次阅读：**快速开始 → 页面使用说明 → 功能与指令示例 → 安全提示**。管理员可直接查看“管理员启动指南”。")
    table = doc.add_table(rows=1, cols=3)
    headers = ["如果你想要", "建议入口", "示例指令"]
    ...
    rows = [
        ("查询和分析财务数据", "对话 / 驾驶舱", "查询2026-05应收账款汇总"),
        ("处理发票和生成凭证", "发票 / 对话", "上传识别一张发票并生成凭证"),
        ("创建和审批报销", "对话", "为E1002创建销售部差旅报销单并提交"),
        ("查看三大财务报表", "报表 / 对话", "生成2026-05资产负债表"),
        ("维护和联调服务", "管理员附录", "执行四组 MCP 冒烟测试"),
    ]
    for row in rows:
...
def main():
    ...
    doc.core_properties.title = "财务通用 Agent 最终用户手册"
    doc.core_properties.subject = "功能说明与使用指南"
